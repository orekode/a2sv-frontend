...
const EditForm = ({ trigger, setTrigger, productId }: { trigger: boolean; setTrigger: (value: boolean) => void; productId: number }) => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`https://a2sv-api.b3trtransit.com/api/products/${productId}`)
  ...
  }, [productId, trigger, setValue])

  const onSubmit = async (data: FormData) => {
    try {
      await axios.put(`https://a2sv-api.b3trtransit.com/api/products/${productId}`, data)
